...
export default function TheamSettings() {
  const {setColor , setMode , currentMode , currentColor  , setThemeSettings } = useStateContext() ; 
  return (
    <div className='bg-half-transparent w-screen fixed nav-item top-9 right-0'>
      <div className='float-right h-screen dark:text-gray-200 bg-white dark:[#484B52] w-400'>
        <div className='flex justify-between items-center p-4 ml-4'>
          <p className='font-semibold text-xl'>Settings</p>
          <button className='text-2xl p-3 hover:drop-shadow-xl hover:bg-light-gray' style={{Color:"rgb(153 , 171 , 180)" , borderRadius:"50%"}}type='button' onClick={()=>{setThemeSettings(false)}}>
            <MdOutlineCancel/>
          </button>
        </div>
        ...
        <div className='flex-col border-t-1 border-color p-4 ml-4'>
          ...
          <div className='flex gap-3'>
            {themeColors.map((item , idx)=>(<TooltipComponent key={idx} content={item.name} position='TopCenter'>
